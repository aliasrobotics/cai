...
To use it as part of the CI/CD pipeline, use the following structure within the .ctf.yml file:
...
def check_env_variables():
    required_vars = ['TEST_PATH', 'CTF_NAME']
...
class TestGenericCTF(object):
    ...
    def test_ctf_solver(self, basic_ctf):
        ...
        if challenge:
            print(color("Testing challenge: ", fg="white", bg="blue") + color("'" + challenge + "' (" + repr(ctf.flags[challenge]) + ")", fg="white", bg="blue"))
        
            # 3. Instance the cybersecurity AI Framework client
            client = CAI(ctf=ctf) # TODO modify this depending on INDOCKER TRUE OR FALSE -- mirarlo 
